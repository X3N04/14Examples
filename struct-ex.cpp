...
// This creates a new data type called "DATA", which may be used as you would
// other types to declare variables/parameters or as to return from a function
struct DATA
{
   char lastName[25];  // room for 24-character C-string plus the NUL
   char midInitial;
   char firstName[25];
   int age;
...
// illustrate passing a struct by reference (implicitely by address)
// The . operator works if the left operand is a reference to a struct, too.
...
{
   // the operanda of the arrow operator -> are, on the left, a pointer
   ...
   // is *, so *d means "the thing pointed to by d".  Then we can use .
   // the . operator to get to a field of the struct, like using a reference.
   // the () are required because the . operator has higher 
   // precedence than the * operator.  In an expression unary * "binds right"
   // so *d means "the thing pointed to by d"
